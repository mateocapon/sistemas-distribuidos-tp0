...
def handle_client_connection(clients_queue, load_bets_queue, waiting_winner_queue):
    """
    ...
    while server_working:
        try:
            ...
            if status == SEND_BETS_INTENTION:
                persist_connection = __receive_bets(client_sock, clients_queue, load_bets_queue)
...
def __receive_bets(client_sock, clients_queue, load_bets_queue):
    """
...
def __send_winners(client_sock):
    ...
    map(lambda bet: bet.document, winning_bets)
